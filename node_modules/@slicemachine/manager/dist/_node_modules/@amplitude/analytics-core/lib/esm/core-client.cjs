"use strict";
Object.defineProperty(exports, Symbol.toStringTag, { value: "Module" });
const tslib_es6 = require("../../../../tslib/tslib.es6.cjs");
const eventBuilder = require("./utils/event-builder.cjs");
const timeline = require("./timeline.cjs");
const resultBuilder = require("./utils/result-builder.cjs");
const messages = require("./messages.cjs");
const returnWrapper = require("./utils/return-wrapper.cjs");
var AmplitudeCore = (
  /** @class */
  function() {
    function AmplitudeCore2(name) {
      if (name === void 0) {
        name = "$default";
      }
      this.initializing = false;
      this.q = [];
      this.dispatchQ = [];
      this.logEvent = this.track.bind(this);
      this.timeline = new timeline.Timeline(this);
      this.name = name;
    }
    AmplitudeCore2.prototype._init = function(config) {
      return tslib_es6.__awaiter(this, void 0, void 0, function() {
        return tslib_es6.__generator(this, function(_a) {
          switch (_a.label) {
            case 0:
              this.config = config;
              this.timeline.reset(this);
              return [4, this.runQueuedFunctions("q")];
            case 1:
              _a.sent();
              return [
                2
                /*return*/
              ];
          }
        });
      });
    };
    AmplitudeCore2.prototype.runQueuedFunctions = function(queueName) {
      return tslib_es6.__awaiter(this, void 0, void 0, function() {
        var queuedFunctions, queuedFunctions_1, queuedFunctions_1_1, queuedFunction, e_1_1;
        var e_1, _a;
        return tslib_es6.__generator(this, function(_b) {
          switch (_b.label) {
            case 0:
              queuedFunctions = this[queueName];
              this[queueName] = [];
              _b.label = 1;
            case 1:
              _b.trys.push([1, 6, 7, 8]);
              queuedFunctions_1 = tslib_es6.__values(queuedFunctions), queuedFunctions_1_1 = queuedFunctions_1.next();
              _b.label = 2;
            case 2:
              if (!!queuedFunctions_1_1.done)
                return [3, 5];
              queuedFunction = queuedFunctions_1_1.value;
              return [4, queuedFunction()];
            case 3:
              _b.sent();
              _b.label = 4;
            case 4:
              queuedFunctions_1_1 = queuedFunctions_1.next();
              return [3, 2];
            case 5:
              return [3, 8];
            case 6:
              e_1_1 = _b.sent();
              e_1 = { error: e_1_1 };
              return [3, 8];
            case 7:
              try {
                if (queuedFunctions_1_1 && !queuedFunctions_1_1.done && (_a = queuedFunctions_1.return))
                  _a.call(queuedFunctions_1);
              } finally {
                if (e_1)
                  throw e_1.error;
              }
              return [
                7
                /*endfinally*/
              ];
            case 8:
              return [
                2
                /*return*/
              ];
          }
        });
      });
    };
    AmplitudeCore2.prototype.track = function(eventInput, eventProperties, eventOptions) {
      var event = eventBuilder.createTrackEvent(eventInput, eventProperties, eventOptions);
      return returnWrapper.returnWrapper(this.dispatch(event));
    };
    AmplitudeCore2.prototype.identify = function(identify, eventOptions) {
      var event = eventBuilder.createIdentifyEvent(identify, eventOptions);
      return returnWrapper.returnWrapper(this.dispatch(event));
    };
    AmplitudeCore2.prototype.groupIdentify = function(groupType, groupName, identify, eventOptions) {
      var event = eventBuilder.createGroupIdentifyEvent(groupType, groupName, identify, eventOptions);
      return returnWrapper.returnWrapper(this.dispatch(event));
    };
    AmplitudeCore2.prototype.setGroup = function(groupType, groupName, eventOptions) {
      var event = eventBuilder.createGroupEvent(groupType, groupName, eventOptions);
      return returnWrapper.returnWrapper(this.dispatch(event));
    };
    AmplitudeCore2.prototype.revenue = function(revenue, eventOptions) {
      var event = eventBuilder.createRevenueEvent(revenue, eventOptions);
      return returnWrapper.returnWrapper(this.dispatch(event));
    };
    AmplitudeCore2.prototype.add = function(plugin) {
      if (!this.config) {
        this.q.push(this.add.bind(this, plugin));
        return returnWrapper.returnWrapper();
      }
      return returnWrapper.returnWrapper(this.timeline.register(plugin, this.config));
    };
    AmplitudeCore2.prototype.remove = function(pluginName) {
      if (!this.config) {
        this.q.push(this.remove.bind(this, pluginName));
        return returnWrapper.returnWrapper();
      }
      return returnWrapper.returnWrapper(this.timeline.deregister(pluginName));
    };
    AmplitudeCore2.prototype.dispatchWithCallback = function(event, callback) {
      if (!this.config) {
        return callback(resultBuilder.buildResult(event, 0, messages.CLIENT_NOT_INITIALIZED));
      }
      void this.process(event).then(callback);
    };
    AmplitudeCore2.prototype.dispatch = function(event) {
      return tslib_es6.__awaiter(this, void 0, void 0, function() {
        var _this = this;
        return tslib_es6.__generator(this, function(_a) {
          if (!this.config) {
            return [2, new Promise(function(resolve) {
              _this.dispatchQ.push(_this.dispatchWithCallback.bind(_this, event, resolve));
            })];
          }
          return [2, this.process(event)];
        });
      });
    };
    AmplitudeCore2.prototype.process = function(event) {
      return tslib_es6.__awaiter(this, void 0, void 0, function() {
        var result, e_2, message, result;
        return tslib_es6.__generator(this, function(_a) {
          switch (_a.label) {
            case 0:
              _a.trys.push([0, 2, , 3]);
              if (this.config.optOut) {
                return [2, resultBuilder.buildResult(event, 0, messages.OPT_OUT_MESSAGE)];
              }
              return [4, this.timeline.push(event)];
            case 1:
              result = _a.sent();
              result.code === 200 ? this.config.loggerProvider.log(result.message) : this.config.loggerProvider.error(result.message);
              return [2, result];
            case 2:
              e_2 = _a.sent();
              this.config.loggerProvider.error(e_2);
              message = String(e_2);
              result = resultBuilder.buildResult(event, 0, message);
              return [2, result];
            case 3:
              return [
                2
                /*return*/
              ];
          }
        });
      });
    };
    AmplitudeCore2.prototype.setOptOut = function(optOut) {
      if (!this.config) {
        this.q.push(this.setOptOut.bind(this, Boolean(optOut)));
        return;
      }
      this.config.optOut = Boolean(optOut);
    };
    AmplitudeCore2.prototype.flush = function() {
      return returnWrapper.returnWrapper(this.timeline.flush());
    };
    return AmplitudeCore2;
  }()
);
exports.AmplitudeCore = AmplitudeCore;
//# sourceMappingURL=core-client.cjs.map
