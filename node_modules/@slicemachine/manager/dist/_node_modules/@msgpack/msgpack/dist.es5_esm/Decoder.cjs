...
const prettyByte = require("./utils/prettyByte.cjs");
...
const CachedKeyDecoder = require("./CachedKeyDecoder.cjs");
...
var sharedCachedKeyDecoder = new CachedKeyDecoder.CachedKeyDecoder();
var Decoder = (
  /** @class */
  function() {
    function Decoder2(extensionCodec, context, maxStrLength, maxBinLength, maxArrayLength, maxMapLength, maxExtLength, keyDecoder) {
      ...
      if (maxStrLength === void 0) {
        maxStrLength = int.UINT32_MAX;
      }
      if (maxBinLength === void 0) {
        maxBinLength = int.UINT32_MAX;
      }
      if (maxArrayLength === void 0) {
        maxArrayLength = int.UINT32_MAX;
      }
      if (maxMapLength === void 0) {
        maxMapLength = int.UINT32_MAX;
      }
      if (maxExtLength === void 0) {
        maxExtLength = int.UINT32_MAX;
      }
    ...
    Decoder2.prototype.appendBuffer = function(buffer) {
      if (this.headByte === HEAD_BYTE_REQUIRED && !this.hasRemaining(1)) {
        ...
        var newData = typedArrays.ensureUint8Array(buffer);
        ...
        return __generator(this, function(_c) {
          switch (_c.label) {
            ...
            case 12:
              ...
              throw new RangeError("Insufficient data in parsing ".concat(prettyByte.prettyByte(headByte), " at ").concat(totalPos, " (").concat(pos, " in the current buffer)"));
          }
        });
      });
    };
      ...
      DECODE:
        while (true) {
          ...
          if (headByte >= 224) {
            ...
            throw new DecodeError.DecodeError("Unrecognized type byte: ".concat(prettyByte.prettyByte(headByte)));
          }
          this.complete();
          ...
          while (stack.length > 0) {
            ...
            if (state.type === 0) {
              ...
              if (!isValidMapKeyType(object)) {
                throw new DecodeError.DecodeError("The type of key must be string or number but " + typeof object);
              }
              if (object === "__proto__") {
                throw new DecodeError.DecodeError("The key __proto__ is not allowed");
              }
              state.key = object;
              state.type = 2;
              continue DECODE;
      ...
      switch (headByte) {
        ...
        default: {
          if (headByte < 160) {
            ...
            throw new DecodeError.DecodeError("Unrecognized array type byte: ".concat(prettyByte.prettyByte(headByte)));
          }
        }
      }
    };
    Decoder2.prototype.pushMapState = function(size) {
      if (size > this.maxMapLength) {
        throw new DecodeError.DecodeError("Max length exceeded: map length (".concat(size, ") > maxMapLengthLength (").concat(this.maxMapLength, ")"));
      }
      this.stack.push({
        type: 1,
        size,
    ...
    Decoder2.prototype.pushArrayState = function(size) {
      if (size > this.maxArrayLength) {
        throw new DecodeError.DecodeError("Max length exceeded: array length (".concat(size, ") > maxArrayLength (").concat(this.maxArrayLength, ")"));
      }
      this.stack.push({
        type: 0,
        size,
    ...
    Decoder2.prototype.decodeUtf8String = function(byteLength, headerOffset) {
      ...
      if (byteLength > this.maxStrLength) {
        throw new DecodeError.DecodeError("Max length exceeded: UTF-8 byte length (".concat(byteLength, ") > maxStrLength (").concat(this.maxStrLength, ")"));
      }
    ...
    Decoder2.prototype.decodeBinary = function(byteLength, headOffset) {
      if (byteLength > this.maxBinLength) {
        throw new DecodeError.DecodeError("Max length exceeded: bin length (".concat(byteLength, ") > maxBinLength (").concat(this.maxBinLength, ")"));
      }
    ...
    Decoder2.prototype.decodeExtension = function(size, headOffset) {
      if (size > this.maxExtLength) {
        throw new DecodeError.DecodeError("Max length exceeded: ext length (".concat(size, ") > maxExtLength (").concat(this.maxExtLength, ")"));
      }
    ...
    Decoder2.prototype.readU64 = function() {
      var value = int.getUint64(this.view, this.pos);
      this.pos += 8;
      return value;
    };
    Decoder2.prototype.readI64 = function() {
      var value = int.getInt64(this.view, this.pos);
      this.pos += 8;
      return value;
    };
...
exports.DataViewIndexOutOfBoundsError = DataViewIndexOutOfBoundsError;
