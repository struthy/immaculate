...
const ChannelNetwork = require("./ChannelNetwork.cjs");
const messages = require("./messages.cjs");
const channelEmitterDefaultOptions = {
  connectTimeout: 2e4,
  requestIDPrefix: "emitter-"
};
class ChannelEmitter extends ChannelNetwork.ChannelNetwork {
  ...
  get channel() {
    if (!this._channel) {
      throw new errors.ChannelNotSetError();
    }
    ...
    return new Promise((resolve, reject) => {
      this._target.addEventListener("load", () => {
        ...
        const receiverReadyTimeout = setTimeout(() => {
          reject(new errors.ConnectionTimeoutError());
        }, this.options.connectTimeout);
        const receiverReadyCallback = async () => {
          clearTimeout(receiverReadyTimeout);
          this.channel = new MessageChannel();
          const request = this.createRequestMessage(types.InternalEmitterRequestType.Connect, receiverOptions);
          const response = await this.postRequest(request, (request2) => {
            this._target.contentWindow.postMessage(request2, "*", [
              this.channel.port2
            ]);
          });
          this.postResponse(messages.createSuccessResponseMessage(this._receiverReady, void 0), (response2) => {
  ...
  async _onPublicMessage(event) {
    ...
    try {
      ...
      if (messages.isRequestMessage(message)) {
        ...
        switch (message.type) {
          case types.InternalReceiverRequestType.Ready:
            ...
            break;
          default:
            this.postResponse(messages.createErrorResponseMessage(message.requestID, void 0), (response) => {
  ...
  postFormattedRequest(type, data, options = {}) {
    if (!this._connected) {
      throw new errors.NotReadyError("Emitter is not connected, use `ChannelEmitter.connect()` first");
    }
    return this.postRequest(this.createRequestMessage(type, data), void 0, options);
  }
}
exports.ChannelEmitter = ChannelEmitter;
exports.channelEmitterDefaultOptions = channelEmitterDefaultOptions;
//# sourceMappingURL=ChannelEmitter.cjs.map
