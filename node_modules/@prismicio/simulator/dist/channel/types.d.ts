...
export type ResponseMessage<TData = void, TError = unknown> = SuccessResponseMessage<TData> | ErrorResponseMessage<TError>;
export type ExtractSuccessResponseMessage<TResponse extends UnknownResponseMessage> = Extract<TResponse, {
    error?: never;
}>;
export type ExtractErrorResponseMessage<TResponse extends UnknownResponseMessage> = Extract<TResponse, {
    data?: never;
}>;
...
export type Transaction<TRequest extends UnknownRequestMessage, TResponse extends UnknownResponseMessage = ResponseMessage<void>> = {
    request: TRequest;
    response: TResponse;
};
export type TransactionMethod<TTransaction extends Transaction<UnknownRequestMessage, UnknownResponseMessage>> = (data: TTransaction["request"]["data"]) => Promise<ExtractSuccessResponseMessage<TTransaction["response"]>>;
export type TransactionHandler<TTransaction extends Transaction<UnknownRequestMessage, UnknownResponseMessage>> = (request: TTransaction["request"], response: {
...
export declare enum InternalEmitterRequestType {
    Connect = "connect"
}
export type InternalEmitterTransactions<TReceiverOptions extends Record<string, unknown> = Record<string, unknown>> = {
...
export declare enum InternalReceiverRequestType {
    Ready = "ready"
}
export type InternalReceiverTransactions = {
    [InternalReceiverRequestType.Ready]: Transaction<RequestMessage<InternalReceiverRequestType.Ready>>;
};
