...
const require$$0$6 = require('readline');
...
async function getConfig() {
  if (!config) {
    ...
    let packageManager = "";
    if (result)
      packageManager = JSON.parse(fs$2.readFileSync(result, "utf8")).packageManager ?? "";
    ...
    else if (!fs$2.existsSync(rcPath))
      config = defaultConfig;
    else
      config = Object.assign({}, defaultConfig, ini.parse(fs$2.readFileSync(rcPath, "utf-8")));
  }
  return config;
}
...
const signal=signals.find(({name})=>require$$0$2.constants.signals[name]===number);
...
function cmdExists(cmd) {
  try {
    require$$0$1.execSync(
      require$$0$2.platform() === "win32" ? `cmd /c "(help ${cmd} > nul || exit 0) && where ${cmd} > nul 2> nul"` : `command -v ${cmd}`
    );
    return true;
  } catch {
    return false;
