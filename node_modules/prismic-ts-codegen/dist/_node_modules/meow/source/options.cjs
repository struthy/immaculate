...
const index = require("../../read-pkg-up/index.cjs");
  ...
  const invalidOptionFilters = {
    flags: {
      keyContainsDashes: {
        ...
        message: (flagKeys) => `Flag keys may not contain '-'. Invalid flags: ${utils.joinFlagKeys(flagKeys, "")}`
      },
      aliasIsSet: {
        filter: ([, flag]) => Object.hasOwn(flag, "alias"),
        message: (flagKeys) => `The option \`alias\` has been renamed to \`shortFlag\`. The following flags need to be updated: ${utils.joinFlagKeys(flagKeys)}`
      },
      choicesNotAnArray: {
        filter: ([, flag]) => Object.hasOwn(flag, "choices") && !Array.isArray(flag.choices),
        message: (flagKeys) => `The option \`choices\` must be an array. Invalid flags: ${utils.joinFlagKeys(flagKeys)}`
      },
      choicesNotMatchFlagType: {
        filter: ([, flag]) => flag.type && Array.isArray(flag.choices) && flag.choices.some((choice) => typeof choice !== flag.type),
        message(flagKeys) {
          const flagKeysAndTypes = flagKeys.map((flagKey) => `(\`${utils.decamelizeFlagKey(flagKey)}\`, type: '${options.flags[flagKey].type}')`);
      ...
      defaultNotInChoices: {
        ...
        message: (flagKeys) => `Each value of the option \`default\` must exist within the option \`choices\`. Invalid flags: ${utils.joinFlagKeys(flagKeys)}`
      }
    }
  };
  const errorMessages = [];
...
const buildOptions = (helpText, options) => {
  ...
  if (!((_a = options.importMeta) == null ? void 0 : _a.url)) {
