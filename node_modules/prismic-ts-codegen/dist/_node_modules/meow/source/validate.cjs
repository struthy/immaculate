...
const validateChoicesByFlag = (flagKey, flagValue, receivedInput) => {
  ...
  if (!receivedInput) {
    if (isRequired) {
      return `Flag \`${utils.decamelizeFlagKey(flagKey)}\` has no value. ${valueMustBeOneOf}`;
    }
    return;
  }
  if (Array.isArray(receivedInput)) {
    ...
    if (unknownValues.length > 0) {
      ...
      return `Unknown ${valuesText} for flag \`${utils.decamelizeFlagKey(flagKey)}\`: \`${unknownValues.join("`, `")}\`. ${valueMustBeOneOf}`;
    }
  } else if (!choices.includes(receivedInput)) {
    return `Unknown value for flag \`${utils.decamelizeFlagKey(flagKey)}\`: \`${receivedInput}\`. ${valueMustBeOneOf}`;
  }
};
...
const reportMissingRequiredFlags = (missingRequiredFlags) => {
  ...
  for (const flag of missingRequiredFlags) {
    console.error(`	${utils.decamelizeFlagKey(flag.key)}${flag.shortFlag ? `, -${flag.shortFlag}` : ""}`);
  }
};
...
exports.checkMissingRequiredFlags = checkMissingRequiredFlags;
exports.checkUnknownFlags = checkUnknownFlags;
