...
const safelyExecute = require("./lib/safelyExecute.cjs");
...
class StartSliceMachineProcess {
  ...
  async run() {
    ...
    if (isTelemetryEnabled) {
      try {
        await setupSentry.setupSentry(this._sliceMachineManager);
      } catch (error) {
      }
    }
...
${allSlices.errors.join("\n\n")}`);
  ...
  async _trackStart() {
    const [adapter, adapterVersion, customTypes, gitProvider, isAdapterUpdateAvailable, isLoggedIn, isSliceMachineUpdateAvailable, isTypeScriptProject, packageManager, simulatorUrl, sliceMachineVersion, slices, versionControlSystem] = await Promise.all([
      ...
      safelyExecute.safelyExecute(() => this._sliceMachineManager.project.checkIsTypeScript()),
      safelyExecute.safelyExecute(() => this._sliceMachineManager.project.detectPackageManager()),
      safelyExecute.safelyExecute(() => this._sliceMachineManager.simulator.getLocalSliceSimulatorURL()),
      safelyExecute.safelyExecute(() => this._sliceMachineManager.versions.getRunningSliceMachineVersion()),
      safelyExecute.safelyExecute(() => this._sliceMachineManager.slices.readAllSlices()),
      safelyExecute.safelyExecute(() => this._sliceMachineManager.project.detectVersionControlSystem())
    ]);
